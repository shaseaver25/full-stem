-- Fix assignment submission status in rpc_assign_lesson_to_class function
-- Change 'assigned' to 'draft' which is the valid status value
...
CREATE OR REPLACE FUNCTION public.rpc_assign_lesson_to_class(
  ...
  p_rubric text DEFAULT NULL::text
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Auto-create submissions for enrolled students with 'draft' status
  INSERT INTO public.assignment_submissions(assignment_id, user_id, status)
  SELECT v_assignment_id, s.user_id, 'draft'
