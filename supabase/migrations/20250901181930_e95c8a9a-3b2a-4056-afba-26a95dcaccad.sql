-- Create demo class and 20 diverse students with personalized learning profiles (Fixed)
...
DECLARE
    ...
    i INTEGER;
BEGIN
    -- Get the first teacher profile for the demo class
    SELECT id INTO demo_teacher_id FROM teacher_profiles LIMIT 1;

    -- Create or find the demo AI class
    SELECT id INTO demo_class_id FROM classes 
    WHERE name = 'AI for High School Students: Understanding, Using, and Creating with Artificial Intelligence';
    ...
    -- Delete existing demo students to ensure clean slate
    DELETE FROM student_profiles WHERE student_id IN (
        SELECT id FROM students WHERE class_id = demo_class_id
    );
    DELETE FROM students WHERE class_id = demo_class_id;
