...
-- First, clean up any existing demo data to avoid conflicts
...
DELETE FROM students WHERE email LIKE '%@student.demo.edu';
DELETE FROM classes WHERE instructor IN ('Sarah', 'Michael', 'Emily') OR name LIKE '%Demo%';
DELETE FROM teacher_profiles WHERE school_name = 'Demo Elementary School' OR school_name = 'Demo Middle School' OR school_name = 'Demo High School';
...
WITH student_lesson_progress AS (
  ...
  AND s.email LIKE '%@student.demo.edu'
)
