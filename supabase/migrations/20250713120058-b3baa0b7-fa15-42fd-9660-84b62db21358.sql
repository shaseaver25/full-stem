...
-- Create developer_settings table for global configuration
...
INSERT INTO public.developer_settings (setting_key, setting_value, description)
...
-- RLS policies for developer_settings
CREATE POLICY "Anyone can view developer settings" 
ON public.developer_settings 
FOR SELECT 
USING (true);

CREATE POLICY "Developers can manage settings" 
ON public.developer_settings 
FOR ALL 
USING (is_developer(auth.uid()));
...
CREATE INDEX IF NOT EXISTS idx_developer_settings_key ON public.developer_settings(setting_key);