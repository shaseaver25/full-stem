...
-- Update assignment_submissions policies for class assignments
CREATE POLICY "Students can view their own class assignment submissions" 
...
USING (
  EXISTS (
    ...
    JOIN public.teacher_profiles tp ON c.teacher_id = tp.id
    WHERE ca.id = assignment_submissions.assignment_id AND tp.user_id = auth.uid()
  )
)
WITH CHECK (
  EXISTS (
...
CREATE INDEX IF NOT EXISTS idx_class_assignments_new_lesson_id ON public.class_assignments_new(lesson_id);